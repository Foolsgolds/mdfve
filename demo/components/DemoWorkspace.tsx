...
export function DemoWorkspace() {
  ...
  // パッケージはクライアント側でのみ読み込む (Shadow DOM / customElements はブラウザ専用)
  useEffect(() => {
    import('@yanqirenshi/markdown.sitter').then(() => setMounted(true))
  }, [])

  // マウント後に Web Components を生成 (一度だけ)
  useEffect(() => {
    if (!mounted || !canvasRef.current || workspaceRef.current) return

    const workspace = document.createElement('markdown-workspace') as HTMLElement & {
      mode: string
      value: string
    }
  ...
  // テーマ変更 — CSS カスタムプロパティを canvas に設定 (Shadow DOM を貫通する)
  useEffect(() => {
